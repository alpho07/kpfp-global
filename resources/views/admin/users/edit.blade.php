...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.users.update", [$user->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('is_verified') ? 'has-error' : '' }}" id="is_verifiedGroup" >
                <label for="institution">Verification Status</label>
                <select name="is_verified" id="is_verified" class="form-control select2">
                    <option value="{{$user->is_verified}}">{{$user->is_verified}}</option>
